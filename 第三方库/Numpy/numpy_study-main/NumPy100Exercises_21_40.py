...
# print(np.array(0) / np.array(0))  #返回nan和警告，runtimeerror
# print(np.array(0) // np.array(0))  #返回nan和警告，runtimeerror
...
print(z)
print(np.copysign(np.ceil(np.abs(z)), z))  #注意ceil在负数时其实是向下取整

# 30. How to find common values between two arrays? (★☆☆)
z1 = np.random.randint(0, 10, 10)
z2 = np.random.randint(0, 10, 10)
print(np.intersect1d(z1, z2))  #返回一个一维，z1和z2形状不需要相同
...
print(np.sqrt(-1) == np.emath.sqrt(-1))  #runtimeerror warning，nan
print(np.sqrt(-1))  #runtimeerror warning，nan
...
print(z // 1)
print(z.astype(int))
print(np.trunc(z))  #丢弃小数部分，负值和ceil不同
...
z = np.linspace(0, 1, 11, endpoint=False)[1:]  #endpoint参数值
print(z)

# 40. Create a random vector of size 10 and sort it (★★☆)
z = np.random.random(10)
z.sort()  #多维的情况会在最小的一维（行）上排序，其他维度不会排序
print(z)
